#!/usr/bin/env python3
"""32x32 synthetic Poisson GP comparison.
...
def environment_info(grid_size: Optional[int] = None) -> dict:
    report = collect_environment_report(grid_size=grid_size)
    report.update(
        {
            "cwd": str(Path.cwd()),
...
def prepare_dirs(cfg: Config) -> tuple[Path, Path]:
    ...
    write_json(environment_path, environment_info(cfg.grid_size))
...
def save_inference(
    ...
    write_metric_schema(output_dir, metrics)
    write_json(
        output_dir / "complete.json",
